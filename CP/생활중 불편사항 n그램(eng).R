...
# 3. 데이터 불러오기
txt=read.xlsx('data/생활중 불편사항_어려움,힘듦,불편_정제.xlsx')
txt1=txt[,-3] #품사 칼럼 삭제
...
# 트라이그램 단어쌍
k=pair_trigram %>%
  filter(word3 %in% c('힘듦', '불편', '어려움')) %>% 
  select(word2,word3,n)
k

# write.xlsx(k, 'data/생활중 불편사항 영어버전.xlsx')

# 영문 번역한 데이터 파일 불러오기
k <- read.xlsx('data/생활중 불편사항 영어버전.xlsx', sheet=3) %>% select(-c(단어2, 단어3))
...
ggraph(graph_trigram, layout = "fr") +        # 레이아웃
  ...
  #                fontface="bold") +           # 볼드
  theme_graph()         +                      # 배경 삭제
  scale_color_discrete(label=c("uncomfortable","difficulty","hard"))+ #범레 레이블 설정
  theme(legend.text= element_text(size = 15, family = "Nanum Gothic"))+ #범례 레이블 서식
